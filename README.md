...
- 🔗 **Records stay ours after Claude forks them** — continuing an imported conversation makes Claude fork it and repoint the record at a session of its own, and the record stopped looking like something this tool wrote, so re-importing added a second entry. The records written for a conversation are now remembered by id, so a forked one is recognised, left alone, and searched for messages you sent there. ([#10](https://github.com/Agentryx-ai/codex-to-claude/pull/10))
